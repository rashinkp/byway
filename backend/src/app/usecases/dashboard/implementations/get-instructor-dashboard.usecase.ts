...
import { IGetInstructorDashboardInput, IInstructorDashboardResponse } from '@/domain/dtos/instructor/instructor-dashboard.dto';
...
export class GetInstructorDashboardUseCase implements IGetInstructorDashboardUseCase {
  ...
  async execute(input: IGetInstructorDashboardInput): Promise<IInstructorDashboardResponse> {
    ...
    const activeCourses = courses.filter((c: any) => 
      c.status === 'PUBLISHED' && !c.deletedAt
    ).length;
    ...
    const completedCourses = courses.filter((c: any) => c.status === 'ARCHIVED').length;

    // Get enrollments for these courses using existing method
    const enrollments: any[] = [];
    for (const courseId of courseIds) {
      const courseEnrollments = await this.enrollmentRepository.findByCourseId(courseId);
    ...
    const topCoursesResponse = await this.courseRepository.getTopEnrolledCourses({ 
      userId: instructorId,
      limit,
      role: "INSTRUCTOR"
    });
    ...
    const topCourses = topCoursesResponse.map((course: any) => ({
      ...
      status: courses.find((c: any) => c.id === course.courseId)?.status || 'DRAFT',
      createdAt: new Date(courses.find((c: any) => c.id === course.courseId)?.createdAt || new Date()),
      lastEnrollmentDate: undefined
    }));

    // Recent students - get unique student IDs and fetch their data
    const studentIds = Array.from(new Set(enrollments.map((e: any) => e.userId)));
    ...
    const recentEnrollments = enrollments
      ...
      .map((e: any) => ({
        ...
        studentName: recentStudents.find((s: any) => s.id === e.userId)?.name || '',
        enrolledAt: e.enrolledAt,
      }));

    // Average rating and total reviews (if available) - including deleted courses
    const averageRating = courses.length ? (courses.reduce((sum: number, c: any) => sum + (c.rating || 0), 0) / courses.length) : 0;
    ...
    return {
      ...
      recentStudents: recentStudents.map((s: any) => {
        const studentEnrollments = enrollments.filter((e: any) => e.userId === s.id);
        return {
          studentId: s.id,
          studentName: s.name,
          email: s.email,
          enrolledCourses: studentEnrollments.length,
          lastEnrollmentDate: studentEnrollments.sort((a: any, b: any) => b.enrolledAt.getTime() - a.enrolledAt.getTime())[0]?.enrolledAt,
