...
export class ListUserChatsUseCase implements IListUserChatsUseCase {
  ...
  async execute(userId: UserId, page: number = 1, limit: number = 10): Promise<PaginatedChatListDTO> {
    return this.chatRepository.findEnhancedChatList(userId, page, limit);
  }
} 