  ...
  if (payload && typeof payload === "object") {
    ...
    if (obj.body) {
      ...
      if (obj.body.data) {
        ...
        if (Array.isArray(obj.body.data.items))
          return obj.body.data.items as T[];
      }
    }
    if (obj.data) {
...
export const sendMessage = (
  ...
  const onMessageSent = handleSuccess();
  const onMessageGeneric = handleSuccess();
  const onNewMessage = handleSuccess();
  const onMessageCreated = handleSuccess();
...
export const getChatHistory = (
  ...
  callback: (history: ChatListItem[]) => void
) => {
