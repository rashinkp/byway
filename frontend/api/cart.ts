...
export async function addToCart(payload: ICartFormData): Promise<ICart> {
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Adding course to cart failed"
    );
  }
}
...
export async function getCart({
  ...
  try {
    ...
    throw new Error(error.response?.data?.message || "Fetching cart failed");
  }
}
...
export async function removeFromCart(courseId: string): Promise<void> {
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Removing course from cart failed"
    );
  }
}
...
export async function clearCart(): Promise<void> {
  try {
    ...
    throw new Error(error.response?.data?.message || "Clearing cart failed");
  }
}
