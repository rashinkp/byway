...
const AnalyticsCard: React.FC<AnalyticsCardProps> = ({
  ...
  return (
    <div
      ...
      style={{ background: "var(--tertiary)", color: "var(--foreground)", maxWidth: "20rem", minWidth: "20rem", borderRadius: "1rem" }}
    >
      {/* Hero Image */}
      ...
      <div className="p-6 flex flex-col">
        ...
        {profileImage && profileName && (
          <div className="flex items-center gap-2 mb-3">
            ...
            />
            <div>
              <h3 className="font-medium text-sm" style={{ color: "var(--primary-foreground)" }}>{profileName}</h3>
              {profileRole && <p className="text-xs" style={{ color: "var(--primary-foreground)" }}>{profileRole}</p>}
            </div>
          </div>
        )}
        {/* Title */}
        ...
        {subtitle && (
          <div className="text-sm text-muted-foreground mb-1 line-clamp-1" style={{ color: "var(--primary-foreground)", opacity: 0.8 }}>
            {subtitle}
          </div>
        )}
        {/* Description */}
