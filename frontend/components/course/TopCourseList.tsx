  ...
  const handleSlide = (dir: 'left' | 'right') => {
    ...
    setTimeout(() => {
      setCurrentIndex((prev) =>
        dir === 'right' ? Math.min(prev + 1, courses.length - 4) : Math.max(prev - 1, 0)
      );
  ...
  return (
    <section className={cn("mb-12 px-2 sm:px-0", className)}>
      ...
      <div className="flex justify-start gap-3 mt-10">
        <Button variant="outline" onClick={() => handleSlide('left')} disabled={!canGoLeft || isAnimating}>
          <ChevronLeft className="w-6 h-6" />
        </Button>
        <Button variant="outline" onClick={() => handleSlide('right')} disabled={!canGoRight || isAnimating}>
