...
export default function CourseSyllabus({
	...
	return (
		<div>
			<h2 className="text-xl font-bold mb-4 text-[var(--color-primary-dark)]">
				Syllabus
			</h2>
			{!lessons || lessons.length === 0 ? (
				<div className="bg-[var(--color-background)] border border-[var(--color-primary-light)]/20 rounded-lg p-6 text-center">
					<p className="text-[var(--color-muted)] text-base font-medium">
						Syllabus is not available. Please contact the team.
					</p>
				</div>
			) : (
				<div className="space-y-4">
					{lessons.map((lesson, index) => (
						<div
							key={index}
							className="border rounded-lg overflow-hidden border-[var(--color-primary-light)]/20"
						>
							<button
