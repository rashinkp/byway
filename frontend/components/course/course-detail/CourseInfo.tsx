			...
			<div className="space-y-4">
					...
					{course?.reviewStats && course.reviewStats.totalReviews > 0 && (
						<div className="flex items-center gap-2">
							...
							<span className="text-sm text-gray-500">
								({course.reviewStats.totalReviews}{" "}
								{course.reviewStats.totalReviews === 1 ? "review" : "reviews"})
							</span>
						</div>
					)}
				</div>
				...
				<div className="flex flex-wrap items-center gap-3">
					...
					{course?.duration && (
						<Badge
							variant="outline"
							className="bg-[var(--color-accent)]/10 text-[var(--color-accent)] border-[var(--color-accent)]/40"
						>
					...
					{course?.price && (
						<Badge
							variant="outline"
							className="bg-[var(--color-warning)]/10 text-[var(--color-warning)] border-[var(--color-warning)]/40"
						>
					...
					{!course?.reviewStats ||
						(course.reviewStats.totalReviews === 0 && (
							<Badge
								variant="outline"
								className="bg-[var(--color-surface)] text-[var(--color-muted)] border-[var(--color-primary-light)]/20"
							>
				...
				{userRole === "ADMIN" && course && (
					<>
						...
						<div className="bg-[var(--color-background)] rounded-lg p-4">
							<h4 className="text-sm font-semibold text-[var(--color-primary-dark)] mb-3">
								Admin Details
							</h4>
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
								...
								<div>
									...
									<p className="font-medium text-[var(--color-primary-light)]">
										{course.adminSharePercentage}%
									</p>
								</div>
								<div>
