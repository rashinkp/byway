      ...
      >
        <div className="inline-flex items-center gap-3 text-[var(--color-danger)] bg-[var(--color-danger)]/10 border border-[var(--color-danger)] rounded-lg px-4 py-3 text-sm font-medium shadow-sm max-w-md">
          ...
          <div className="flex flex-col items-start">
            ...
            <span className="text-[var(--color-danger)] font-normal">{errorMessage}</span>
          </div>
          {onRetry && (
            <button
              onClick={onRetry}
    ...
    >
      <div className="bg-[var(--color-surface)] rounded-lg shadow-sm p-8 w-full text-center border border-[var(--color-danger)]">
        <div className="flex justify-center mb-6">
          <AlertCircle size={48} className="text-[var(--color-danger)]" aria-hidden="true" />
        </div>
        <h1 className="text-2xl font-bold text-[var(--color-primary-dark)] mb-2">{title}</h1>
        {code && <div className="text-sm text-[var(--color-danger)] mb-2">Error Code: {code}</div>}
